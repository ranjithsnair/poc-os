...
//
// The 64-bit ELF header/program-header shapes below aren't just a
// 32-bit shape with wider fields - a couple of fields move, to keep
// every field naturally aligned (notably p_flags, which is the
// *second* field of Elf64_Phdr instead of the second-to-last).
// boot/bootmain.c parses this same shape too - it stays a 32-bit
// program itself (see the Makefile's BOOTCC), but reads this
// 64-bit-shaped header since that's the shape the kernel it's loading
// was linked with.
struct elfhdr {
