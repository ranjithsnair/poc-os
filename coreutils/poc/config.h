/* poc-os note: this file is NOT what a cross-compile ./configure run
 * for poc-os would produce (this build never runs autoconf/automake
 * at all - see the Makefile's coreutils/ comment). It started as the
 * output of a real, native (non-cross, macOS) ./configure --disable-
 * nls --disable-threads run - the fastest way to get a realistic,
 * complete baseline (thousands of #defines) instead of writing them
 * from scratch - then hand-corrected wherever that native host's
 * answer doesn't hold for poc-os's musl/kernel: NLS forced off, the
 * HAVE_DECL_*_UNLOCKED (BSD/glibc "unlocked stdio") family forced to
 * 0, and a couple of small poc-os-only additions appended at the
 * bottom (unreachable() - see coreutils/poc/poc_prelude.h for its
 * larger siblings). If a real coreutils utility ever fails to build
 * or misbehaves because of a wrong, still-native-only HAVE_ macro
 * lingering in here, that's the first place to look - this file is
 * trusted, not authoritative.
 */
/* Below this point: original autoconf-generated content (lib/config.h,
 * from config.hin/configure.ac), on the native host it was captured on. */
...
/* Define to 1 if you have the declaration of 'strmode', and to 0 if you
   don't. musl has no strmode(3) of its own (a BSD/macOS extension, which
   is where this config.h was otherwise adapted from - see this file's
   own st_atim/st_birthtime comments above for the same class of
   mismatch) - 0 here lets gnulib's own lib/filemode.c provide the real
   definition instead of assuming libc already has one. */
...
/* Define to 1 if 'st_atim.tv_nsec' is a member of 'struct stat'. musl's own
 * struct stat (musl/arch/x86_64/bits/stat.h) uses this Linux/POSIX nested-
 * timespec layout, not the BSD/macOS st_atimespec one above - this config.h
 * was hand-adapted from a native (Darwin) ./configure run (see the
 * Makefile's own comment on coreutils/poc/config.h), which got this
 * backwards for a musl target. */
 ...
 * musl has no birth-time tracking of any kind (no st_birthtimespec, no
 * st_birthtim) - correctly leaving every HAVE_STRUCT_STAT_ST_BIRTHTIME*
 * flag undefined, unlike the Darwin ./configure run this config.h was
 * otherwise adapted from (see this file's own st_atim comment above). */
...
/* --- poc-os-specific additions below, not from autoconf --- */
/* gnulib's own C23 stddef.h replacement (lib/stddef.in.h) would
 * define this fallback; we use musl's real <stddef.h> directly
 * instead (see the Makefile's -I order), which doesn't, so add the
 * same fallback gnulib itself would have used for a GCC-family
 * compiler. */
#define unreachable() __builtin_unreachable()

/* Override the native (macOS) config.h's *_UNLOCKED decls to 0 - poc-os's
 * musl has none of the BSD/glibc "unlocked stdio" fast-path functions
 * (getc_unlocked, clearerr_unlocked, ...), so lib/unlocked-io.h should
 * fall back to its own portable macros (clearerr_unlocked(x) ->
 * clearerr(x), etc), same as any system lacking them. */
