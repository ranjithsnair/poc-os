/* poc-os note: this file is NOT what a cross-compile ./configure run for
 * poc-os would produce (this build never runs autoconf/automake at all -
 * see the Makefile's coreutils/ and bash/ comments for why). It started
 * as the output of a real, native (non-cross, macOS) `./configure
 * --enable-tiny --disable-nls --disable-utf8` run - the fastest way to
 * get a realistic, complete baseline (thousands of #defines) instead of
 * writing them from scratch, exactly like coreutils/poc/config.h's own
 * comment describes doing for coreutils - then hand-corrected wherever
 * that native host's answer doesn't hold for poc-os's musl/kernel. If a
 * real nano build step ever fails or misbehaves because of a wrong,
 * still-native-only HAVE_ macro lingering in here, that's the first
 * place to look - this file is trusted, not authoritative.
 */
...
/* poc-os: forced off - see nano/poc/config.h's own top-of-file comment.
 * key_defined()/tigetstr() are real terminfo-database queries; poc-os's
 * curses/ (Stage 2) has no terminfo at all. */
...
/* poc-os: forced off - see nano/poc/config.h's own top-of-file comment.
 * The native host this baseline was captured on has real ncurses;
 * poc-os has curses/include/curses.h (Stage 2) instead, which
 * nano/src/definitions.h only reaches for once this is undefined. */
...
#define PACKAGE_STRING "GNU nano 6.4"
