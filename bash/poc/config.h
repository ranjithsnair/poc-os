...
/* #undef BANG_HISTORY */  /* poc-os: HISTORY itself is off (see below) -
   config-bot.h's own BANG_HISTORY-requires-HISTORY fixup would silently
   force HISTORY back on otherwise. */
...
/* poc-os/musl: struct stat's st_atim/st_mtim/st_ctim are plain
 * struct timespec (musl/arch/x86_64/bits/stat.h) - the POSIX-standard
 * shape, not Darwin/BSD's st_atimespec naming the native macOS
 * configure run this was adapted from detected. */
...
/* #undef HAVE_ICONV */  /* poc-os/musl here is permanently C/POSIX locale, no real iconv - see coreutils/poc/coreutils_shims.c c32isprint() for the same call */
...
/* #undef HAVE_SYSLOG */  /* poc-os has no syslog */
...
/* #undef HAVE_LIBDL */  /* poc-os has no dlopen (no loadable-builtins support) */
