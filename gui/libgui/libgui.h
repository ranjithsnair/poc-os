...
// Requests a w x h surface with the given title (may be NULL). On
// success, c->surface is mmap()'d and ready to draw into via gfx.h's
// functions; c->surface_id identifies it to the compositor for
// gui_commit()/gui_destroy(). Returns 0 on success, -1 on failure.
int gui_create_surface(struct gui_conn *c, int w, int h, const char *title);
